						    ...
						    gboolean interp)
{
	...
	const char *function_name =
	    interp ? interp_function_name : nn_function_name;
...
firtree_engine_get_constant_for_kernel_argument(GValue * kernel_arg)
{
	...
	if (type == G_TYPE_FLOAT) {
		...
		FirtreeVec2 *vec_box =
		    (FirtreeVec2 *) g_value_get_boxed(kernel_arg);
		...
		FirtreeVec3 *vec_box =
		    (FirtreeVec3 *) g_value_get_boxed(kernel_arg);
		std::vector < llvm::Constant * >vec_vals;
		vec_vals.
		    push_back(llvm::ConstantFP::
			      get(llvm::Type::FloatTy, (double)vec_box->x));
		...
		FirtreeVec4 *vec_box =
		    (FirtreeVec4 *) g_value_get_boxed(kernel_arg);
		std::vector < llvm::Constant * >vec_vals;
		vec_vals.
		    push_back(llvm::ConstantFP::
			      get(llvm::Type::FloatTy, (double)vec_box->x));
...
void
firtree_engine_create_standard_optimization_passes(llvm::PassManager * PM,
